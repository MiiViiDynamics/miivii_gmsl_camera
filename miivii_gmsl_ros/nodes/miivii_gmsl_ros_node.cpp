...
class miivii_gmsl_ros_node {
 public:
  ...
  miivii_gmsl_ros_node() : node_("~") {
    ...
    m_gmsl_camera = new MVGmslCamSDK(dev_name, camCount_,
                                     camWidth_,
                                     camHeight_, fps_);
    ...
    for (int i = 0; i < camCount; i++) {
      gmsl_image_pub_[i] =
          it.advertiseCamera((domain_name + camera_topic_list[i]).c_str(), 1);

      ROS_INFO("camera_cali_file_list %d %s", i,
               camera_cali_file_list[i].c_str());
  ...
  bool grab_and_send_image() {
    uint64_t timestap;
    if (m_gmsl_camera->MVGmslGetImage(gmsl_outbuf, timestap)) {
      ros::Time Ts;
      if (enable_sync_ == 1) {
